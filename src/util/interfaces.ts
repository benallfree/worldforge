...
export type ValueNarrowingIteratee<I, O> = (item: any, key: StringifiedKey<I>) => item is O
export type KeyNarrowingIteratee<I, O> = (item: I[keyof I], key: any) => key is O
...
export type IfCouldBe<T1, T2, If, Else = never> = Narrow<T1, T2> extends never ? Else : If
export type IfIndexType<T, If, Else = never> = string extends T ? If : number extends T ? If : Else
...
export type Drop1Arg<T extends Function> = T extends (arg1: any, ...rest: infer A) => infer R
  ? (...rest: A) => R
  : never
